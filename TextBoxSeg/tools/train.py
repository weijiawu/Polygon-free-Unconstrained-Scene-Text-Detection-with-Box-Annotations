...
class Trainer(object):
    def __init__(self, args):
        ...
        data_kwargs = {'root': cfg.DATASET.ROOT, 'transform': input_transform, 'base_size': cfg.TRAIN.BASE_SIZE}
        train_dataset = get_segmentation_dataset(cfg.DATASET.NAME, split='train',
                                                 mode='train', debug=False, **data_kwargs)
        ...
        # ce    L1_loss
        cfg.SOLVER.LOSS_NAME = 'ce'
        self.criterion_s = get_segmentation_loss(cfg.MODEL.MODEL_NAME).to(self.device)
    ...
    def train(self):
        ...
        for (images, targets, skeleton, _, _) in self.train_loader:
            # import cv2
            # import numpy as np
            # img = (np.array(images[0].numpy().transpose(1, 2, 0))*[0.229, 0.224, 0.225]+[0.485, 0.456, 0.406])*255
            # mask = np.array(targets[0].numpy())
            # dist_img_ = np.array(skeleton[0].numpy())
            ...
            outputs , skeleton_pre = self.model(images)

            loss_dict = self.criterion(outputs, targets)

            skeleton = skeleton.float()
            # valid = torch.ones_like(skeleton)
            lossb_dict = self.criterion_s(skeleton_pre[0], skeleton)

            weight_boundary = 1
            # lossb = weight_boundary * lossb_dict

            # losses = sum(loss for loss in loss_dict.values()) + \
            #          sum(loss for loss in lossb_dict.values())

            losses = sum(loss for loss in loss_dict.values()) + weight_boundary * lossb_dict
