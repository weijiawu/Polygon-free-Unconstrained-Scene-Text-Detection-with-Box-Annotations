...
debug_flag = True
img_save_path = './demo/trash/Total_image'
...
def parse_img_gt(img_path, txt_path):
    ...
    return img, boxes, ptss, confidence
...
def demo():
    ...
    if os.path.exists('/data/data_weijiawu/Total_Text/DB/'):
        ...
        ic15_root_path = '/data/data_weijiawu/Total_Text/DB/'
    ctw_train_data = ic15_root_path + 'train_images'
    ctw_train_gt = ic15_root_path + 'train_gts'
    ...
    for idx,(img_path,gt_path) in enumerate(zip(image_list, annotation_list)):
        ...
        if os.path.exists(gt_path_) and os.path.exists(img_path_):
            img, boxes, ori_box, confident = parse_img_gt(img_path_, gt_path_)
            img = np.array(img)
            #遍历box
            ...
            for j, (box,confident_) in enumerate(zip(boxes,confident)):
