...
def demo():
    ...
    if os.path.exists('/data/data_weijiawu/ctw1500/train/'):
        ...
        ic15_root_path = '/data/data_weijiawu/ctw1500/train/'
    ic15_train_data = ic15_root_path + 'text_image'
    ic15_train_gt = ic15_root_path + 'text_label_curve'
    assert os.path.exists(ic15_train_data) and os.path.exists(ic15_train_gt)

    image_list = os.listdir(ic15_train_data)
    annotation_list = ['{}.txt'.format(img_name.replace('.jpg', '')) for img_name in image_list]
...
def parse_img_gt(img_path, txt_path):
    ...
    for line in lines:
        ...
        gt = line.split(',')
        oriented_box = [int(gt[i]) for i in range(4,len(gt))]
        oriented_box = np.asarray(oriented_box)

        xs = oriented_box.reshape(int(len(oriented_box) / 2), 2)[:, 0] + int(gt[0])
        ys = oriented_box.reshape(int(len(oriented_box)/2), 2)[:, 1] + int(gt[1])
