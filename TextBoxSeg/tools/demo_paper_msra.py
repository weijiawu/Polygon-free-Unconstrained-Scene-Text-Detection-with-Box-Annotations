...
def demo():
    ...
    msra_train = os.path.abspath('/mnt/lustre/share_data/xieenze/wuweijia/MSRA-TD500/MSRA-TD500')
    ...
    for idx, img_path_one in enumerate(img_paths):
        ...
        if os.path.exists(gt_path) and os.path.exists(img_path):
            ...
            for j, box in enumerate(boxes):
                ...
                cv2.drawContours(img, [rbox], 0, (255, 0, 0), 3)

                mask_1 = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
                cv2.fillPoly(mask_1, [rbox], 1)
                img = mask_image(img, mask_1)

                # cv2.drawContours(img, [np.array(ori_box[j], dtype=np.int32)[:8].reshape(4, 2)], 0, (0, 0, 255), 3)

            cv2.imwrite('{}/img{}.png'.format(img_save_path, idx), img)
                # if debug_flag:
                #     if is_ignore == 1:
