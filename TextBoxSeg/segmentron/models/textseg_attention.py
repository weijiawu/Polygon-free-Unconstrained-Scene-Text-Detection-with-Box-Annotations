...
__all__ = ['textseg_attention']

def _resize_image(img, h, w):
    return F.interpolate(img, size=[h, w], mode='bilinear', align_corners=True)

@MODEL_REGISTRY.register(name='textseg_attention')
class textseg_attention(SegBaseModel):
    def __init__(self):
        super(textseg_attention, self).__init__()
    ...
    def forward(self, x):
        ...
        outputs.append(x)
        outputs_b.append(x_b.sigmoid())#.sigmoid())
