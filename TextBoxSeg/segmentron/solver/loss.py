...
class bce_loss(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        predict = predict.contiguous().view(predict.shape[0], -1)
        target = target.contiguous().view(target.shape[0], -1)
        y_true_f = ((target == 0) * 1.0).float()
        y_true_a = ((target != 0) * 1.0).float()
        predict = predict.clamp(min=1e-7, max=1 - 1e-7)
        P_BCE_LOSS = -(y_true_a * (1 - abs(predict - target).clamp(min=1e-7, max=1 - 1e-7)).log() + \
                       y_true_f * (1 - abs(predict - target).clamp(min=1e-7, max=1 - 1e-7)).log())
        Weight_P_BCE_LOSS = (P_BCE_LOSS).mean()
        return Weight_P_BCE_LOSS
...
def get_segmentation_loss(model, use_ohem=False, **kwargs):
    ...
    elif cfg.SOLVER.LOSS_NAME == "ce":
        return bce_loss()
    elif cfg.SOLVER.LOSS_NAME == "L1_loss":
