import evaluation.msra.file_util as file_util
...
gt_root =   '/data/data_weijiawu/TD500/Test_gt/'
...
def get_msra_result(pred_path_root,gt_path_root):
    ...
    for pred_path in pred_list:
        ...
        gt_path = os.path.join(gt_path_root ,pred_path.split('/')[-1].split('.')[0].split('res_')[-1] + '.gt')
...
if __name__ == '__main__':
    ...
    for pred_path in pred_list:
        count = count + 1
        preds = get_pred(pred_path)
        gt_path = gt_root + pred_path.split('/')[-1].split('.')[0].split('res_')[-1] + '.gt'
        gts, tags = get_gt(gt_path)
