...
class ICDAR15(data.Dataset):
    def __init__(self, data_dir,is_pseudo, data_shape: int = 640, n=6, m=0.5):
        self.data_list = self.load_data(data_dir,is_pseudo)
    ...
    def load_data(self, data_dir: str,is_pseudo:bool) -> list:
        data_list = []
        for x in glob.glob(data_dir + '/train_img/*.jpg', recursive=True):
            d = pathlib.Path(x)
            if is_pseudo:
                label_path = os.path.join(data_dir, 'ch4_training_localization_transcription_gt_pseudo', ('gt_' + str(d.stem) + '.txt'))
                # print("trianing with pseudo label")
            else:
