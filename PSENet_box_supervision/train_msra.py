...
def train_epoch(net, optimizer, scheduler, train_loader, device, criterion, epoch, all_step,  logger):
    ...
    for idx, (images, labels, training_mask) in enumerate(train_loader):
        ...
        for i in range(config.kernel_num-1):
            kernel_i = kernels[:, i, :, :]
...
def main():
    ...
    train_data = MSRA_TD500(config.msra_path,
                            config.hust_path,
                            is_box_pseudo = config.is_box_pseudo,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume_from', '-r', type=str, default="/home/xjc/Desktop/CVPR_SemiText/SemiText/PSENet_self_training/workdirs/Synthtext/6kernal_epoch_1.0_model.pth", help='')
